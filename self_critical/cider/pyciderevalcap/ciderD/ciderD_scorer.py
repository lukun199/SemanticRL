...
class defaultdict:
    def __init__(self, _type):
        self.dict = dict()
        if _type == float: self.default_v = 0.0
        elif _type == int: self.default_v = 0

    def __getitem__(self, key):
        if key in self.dict:
            return self.dict[key]
        else: return self.default_v
...
class CiderScorer(object):
    ...
    def copy_empty(self):
        new = CiderScorer(df_mode="scratch", n=self.n, sigma=self.sigma)
        new.df_mode = self.df_mode
        new.ref_len = self.ref_len
        new.document_frequency = self.document_frequency
        return new

    def __init__(self, cache=None, df_mode="scratch", test=None, refs=None, n=4, sigma=6.0):
        ''' singular instance '''
        self.n = n
        self.sigma = sigma
        self.crefs = []
        ...
        if cache:
            self.update_df(cache)

    def update_df(self, refs):
        for ref in refs:
            self.crefs.append(cook_refs(ref))
